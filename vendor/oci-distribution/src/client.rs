...
use crate::config::{Architecture, ConfigFile, Os};
use crate::errors::*;
use crate::manifest::{
...
impl Client {
    ...
    /// Get available platforms for an image.
    ///
    /// This method fetches the manifest for an image and returns the available platforms.
    /// For multi-platform images (image indexes), it returns all platforms from the index.
    /// For single-platform images, it fetches the config blob to determine the platform.
    pub async fn get_image_platforms(
        ...
        let (manifest, _digest) = self._pull_manifest(image).await?;

        match manifest {
            OciManifest::Image(img_manifest) => {
                // Single platform image - fetch config to get platform info
                let mut config_data = Vec::new();
                self.pull_blob(image, &img_manifest.config, &mut config_data).await?;

                // Parse config as ConfigFile
                let config_str = std::str::from_utf8(&config_data)
                    .map_err(|e| OciDistributionError::ConfigConversionError(e.to_string()))?;
                let config_file: ConfigFile = serde_json::from_str(config_str)
                    .map_err(|e| OciDistributionError::ConfigConversionError(e.to_string()))?;

                // Convert Os and Architecture enums to strings
                let os = match config_file.os {
                    Os::Linux => "linux",
                    Os::Windows => "windows",
                    Os::Darwin => "darwin",
                    Os::Freebsd => "freebsd",
                ...
                let arch = match config_file.architecture {
            ...
            OciManifest::ImageIndex(index) => {
                ...
                let platforms: Vec<(String, String)> = index.manifests
                    .iter()
                    .filter_map(|entry| {
                        entry.platform.as_ref().map(|p| {
                            (p.os.clone(), p.architecture.clone())
                        })
                    })
    ...
    /// Pushes the manifest for a specified image and returns the digest
    ///
    /// Returns a tuple of (pullable manifest URL, manifest digest)
    pub async fn push_manifest_and_get_digest(&self, image: &Reference, manifest: &OciManifest) -> Result<(String, String)> {
        let mut headers = HeaderMap::new();
        let content_type = manifest.content_type();
        headers.insert("Content-Type", content_type.parse().unwrap());
        ...
        let url = self.push_manifest_raw(image, body, manifest.content_type().parse().unwrap())
            .await?;

        Ok((url, manifest_digest))
    }
